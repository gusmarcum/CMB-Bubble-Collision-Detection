"""
Extract negative-only null-control patches from the real Planck SMICA map using
the same coordinate pool and split geometry as a generated training dataset.
"""
...
from phase2_generate_training import NSIDE_WORKING, PATCH_PIX, RESO_ARCMIN, project_patch
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract real-SMICA null-control patches using a generated dataset's coordinate pool and split.",
...
def default_output_path(training_h5, map_name):
    training_h5 = os.path.abspath(training_h5)
    return os.path.join(os.path.dirname(training_h5), f"{map_name}_null_controls.h5")
...
def main():
    ...
    output_h5 = os.path.abspath(args.output_h5) if args.output_h5 else default_output_path(training_h5, args.map_name)
    ...
    with h5py.File(training_h5, "r") as src:
        ...
        coord_val_idx = np.asarray(splits["coord_val_idx"][:], dtype=np.int32)
    ...
    if args.split in {"all", "val"}:
        split_sources.append((1, coord_val_idx))
    ...
    with h5py.File(output_h5, "w") as dst:
        ...
        val_idx = np.flatnonzero(metadata["split_tag"] == 1).astype(np.int64)
